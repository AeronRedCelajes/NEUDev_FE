...
import '/src/style/StudentNavbar.css';
...
export const StudentNavbar = () => {
  ...
  const getBackLink = () => {
    ...
    if (path === '/student/dashboard' || path === '/student/profile') return null;

    // If inside a class route:
    if (classID && path.includes(`/student/class/${classID}`)) {
      // If you're in an activity detail page (assessment, leaderboard, or items)
      if (path.includes('/activity/') && actID) {
        return `/student/class/${classID}/activity`;
      }
      // If on the bulletin page, you might decide to go back to the class's activity list
      if (path.includes('student-bulletin')) {
        return `/student/class/${classID}/activity`;
      }
    }
  ...
  const handleBack = () => {
    ...
    navigate(backLink || '/student/dashboard');
  };
...
// THIS IS THE FUNCTION TO JUST SIMPLY SHOW THE NOTIFICATION WITHOUT MAKING THEM AS READ
  ...
  return (
    <>
      {/* Render sidebar only when on dashboard */}
      ...
      <Navbar expand='lg' fixed='top' className='profile-student-navbar'>
        {/* Show the sidebar toggle (faBars) only on the dashboard; otherwise, show a back arrow */}
        {location.pathname === '/student/dashboard' ? (
